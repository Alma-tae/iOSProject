...
class RootViewController: UITableViewController {
    ...
    override func viewDidLoad() {
      ...
      //다운로드 받을 URL을 생성
        ...
        //XMLParserDelegate의 메소드를 구현한 객체를 설정
        if success == true{
            self.title = "Parsing succeed"
        }else{
            self.title = "Parsing failed"
        }
    ...
    override func numberOfSections(in tableView: UITableView) -> Int { //열
        return 1
    }

    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int { //행
        return books.count
    }
...
extension RootViewController : XMLParserDelegate{
    ...
    func parser(_ parser: XMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [String : String] = [:]) {
        //객체를 생성하는 키를 만났을 때 파싱한 데이터를 저장할 객체 생성
        //속성이 있다면 속성을 저장
        if elementName == "Book"{
            book = Book()
